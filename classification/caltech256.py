...
path = 'C:\Users/boogi/preprocessing'
df = pd.read_csv('%s/caltech256X.csv'%path)
dfy = np.load('%s/caltech256Y.npy'%path)
df.drop('Unnamed: 0', axis=1, inplace=True)
dyf.drop('Unnamed: 0', axis=1, inplace=True)

X_train = np.array(df).astype('float32') / 255.
